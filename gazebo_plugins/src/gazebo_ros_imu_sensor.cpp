...
void gazebo::GazeboRosImuSensor::Load(gazebo::sensors::SensorPtr sensor_, sdf::ElementPtr sdf_)
{
  ...
  if (!this->sdf->HasElement("compensateGravity"))
  {
    ROS_DEBUG("imu plugin missing <compensateGravity>, defaults to true");
    this->compensate_gravity = true;
  }
  else
    this->compensate_gravity = this->sdf->GetElement("compensateGravity")->Get<bool>();
